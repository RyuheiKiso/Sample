...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		common.Warn("[REST] Invalid method: %s", r.Method)
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		common.Error("[REST] JSON decode error: %v", err)
	...
	if err != nil {
		common.Error("[REST] DB接続エラー: %v", err)
	...
	if err != nil {
		common.Warn("[REST] 認証失敗: %s", req.Username)
		w.WriteHeader(http.StatusUnauthorized)
		json.NewEncoder(w).Encode(LoginResponse{Error: "Invalid credentials"})
		return
	}
	common.Info("[REST] 認証成功: %s", req.Username)
